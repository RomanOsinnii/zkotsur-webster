...
export function EditorProperties(props: Props) {
  const { isReadOnly, frameWidthInput, setFrameWidthInput, commitFrameWidth, frameHeightInput, setFrameHeightInput, commitFrameHeight, activeFrame, updateFrameBackgroundMode, updateFrameBackground, updateFrameBackgroundOpacity, getFrameStops, updateFrameGradientStop, removeFrameGradientStop, addFrameGradientStop, activeName, selectedObject, removeSelected, opacity, updateOpacity, strokeColor, strokeWidth, updateStrokeColor, updateStrokeWidth, rotation, updateRotation, elementWidth, elementHeight, updateElementWidth, updateElementHeight, updateImageCornerRadius, fillLayers, activeFillLayer, selectFillLayer, colorWithOpacity, createGradientPreview, addFillLayer, removeFillLayer, fillMode, applyFillMode, fillColor, fillOpacity, updateFill, updateFillOpacity, gradientStops, updateGradientStop, removeGradientStop, addGradientStop, canEditCorners, cornerFields, cornerRadii, updateCornerRadius, isTextSelected, fontFamily, fontOptions, updateFontFamily, fontSize, updateFontSize, textAlign, updateTextAlign } = props;
  ...
  return (
    <aside className="properties" aria-label="Object properties">
      ...
      <section className="tool-section"><h2>Appearance</h2><label className="field"><span>Opacity</span><input max="1" min="0" onChange={(event) => updateOpacity(Number(event.target.value))} step="0.05" type="range" value={opacity} /></label><div className="size-row"><label className="field compact-field"><span>Width</span><input min="1" onChange={(event) => updateElementWidth(Number(event.target.value))} type="number" value={elementWidth} /></label><label className="field compact-field"><span>Height</span><input min="1" onChange={(event) => updateElementHeight(Number(event.target.value))} type="number" value={elementHeight} /></label></div><div className="size-row"><label className="field compact-field"><span>Border</span><input max={maxBorderWidth} min="0" onChange={(event) => updateStrokeWidth(Number(event.target.value))} type="number" value={strokeWidth} /></label><label className="field compact-field"><span>Border color</span><input disabled={!selectedObject} onChange={(event) => updateStrokeColor(event.target.value)} type="color" value={strokeColor} /></label></div><label className="field compact-field"><span>Rotation</span><input max="360" min="-360" onChange={(event) => updateRotation(Number(event.target.value))} type="number" value={Math.round(rotation)} /></label>{isImageSelected ? <label className="field compact-field"><span>Image radius</span><input min="0" onChange={(event) => updateImageCornerRadius(Number(event.target.value))} type="number" value={Math.round(cornerRadii.topLeft)} /></label> : null}</section>
